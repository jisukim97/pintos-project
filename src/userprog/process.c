...
{
  char *fn_copy, *fn_copy2, *name, *save_ptr;
  tid_t tid;
  struct process *pcb;
  ...
  if (pcb->pid != PID_ERROR)
    list_push_back(thread_get_children(), &pcb->child_elem);
...
start_process(void *pcb_)
{
  ...
  /* Set current thread's pcb with pcb_. */
  thread_set_pcb(pcb);
  ...
  success = pcb->is_loaded = load(file_name, &if_.eip, &if_.esp);
...
/* Returns current process's child process with pid. */
struct process *
process_get_child(pid_t pid)
{
  ...
  for (e = list_begin(children); e != list_end(children); e = list_next(e))
  {
    struct process *pcb = list_entry(e, struct process, child_elem);
...
/* Remove current process's child and free its space when it is terminated. */
